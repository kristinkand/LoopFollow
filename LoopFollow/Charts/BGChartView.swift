...
private struct MainBGChart: View {
    ...
    /// stands on the mark. The pill stacks every BG reading in the block
    /// first (each already time-first, see bgPillText), then every
    /// treatment in it, then any band at the mark, so it is constant across
    /// the block. The indicator's height comes from the reading nearest the
    /// mark, else the nearest treatment, else the band; an empty block shows
    /// nothing.
    private func selectionAnchor(for selected: Date) -> SelectionAnchor? {
        ...
        // Readings lead the stack (see bgPillText), then whatever treatments -- an SMB, a
        // bolus, etc. -- fall in the same block.
        var texts = readings.map(bgPillText) + treatments.map(\.pillText)
