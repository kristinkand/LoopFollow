...
extension Storage {
    ...
    func migrateStep1() {
        ...
        // Move the legacy app-group arrays into the standard keys; migrateStep10
        // (or migrateInfoDisplayItems) then folds them into infoDisplayItems.
        move(UserDefaultsValue<[Int]>(key: "infoSort",
                                      default: InfoType.allCases.map { $0.sortOrder }),
             into: StorageValue<[Int]>(key: "infoSort", defaultValue: InfoType.allCases.map { $0.sortOrder }))

        move(UserDefaultsValue<[Bool]>(key: "infoVisible",
                                       default: InfoType.allCases.map { $0.defaultVisible }),
             into: StorageValue<[Bool]>(key: "infoVisible", defaultValue: InfoType.allCases.map { $0.defaultVisible }))
